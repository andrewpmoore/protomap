...
class ChartPressureObservations extends ConsumerStatefulWidget {
  const ChartPressureObservations({Key? key}) : super(key: key);

  @override
  ChartPressureObservationsState createState() => ChartPressureObservationsState();
}

class ChartPressureObservationsState extends ConsumerState<ChartPressureObservations> {
  ...
  Widget build(BuildContext context,) {
    ...
    // }
    //
    // setMinAndMaxPressure(weatherProv);
    ...
    //       child: Row(
    //         children: [
    //           Image.asset('$imagePath/pressure.png', width: 24, height: 24, color: context.colorScheme.tertiary,),
    ...
    //             children: [
    //               const Text('Pressure'),
    //               Text('Pressure: ↓ ${Conversion.pressure(minPressure, ref.read(settingsProvider).units).toInt()} ${ref.read(settingsProvider).units.pressure} @ ${Conversion.time(minTime, ref.read(settingsProvider).units)}   ↑ ${Conversion.pressure(maxPressure, ref.read(settingsProvider).units).toInt()} ${ref.read(settingsProvider).units.pressure} @ ${Conversion.time(maxTime, ref.read(settingsProvider).units)}', style: context.style.labelSmall,),
    //             ],
    //           ),
    //         ],
    //       ),
  ...
  LineChartData mainData(WundergroundTodayObservations? todayObservations) {
    ...
    for (int i=0; i<todayObservations!.observations.length; i++){
      ...
      flSpotList.add(FlSpot(i.toDouble(), observation.metric!['pressureMax']!));
    }
  ...
  List<Color> getGradientColors(WeatherAreasProvider weatherProv) {
    List<Color> gradientColors = [Colors.pinkAccent, Colors.pinkAccent];
    return gradientColors;
  }



  void setMinAndMaxPressure(WeatherAreasProvider weatherProv) {
    for (int i=0; i<weatherProv.singleWeatherArea!.todayObservations!.observations.length; i++){
      ...
      double highPressure = observation.metric!['pressureMax']!;
      if (highPressure<minPressure){
        minPressure = highPressure;
        minTime = observation.obsTimeLocal;
      }
      if (highPressure>maxPressure){
        maxPressure = highPressure;
