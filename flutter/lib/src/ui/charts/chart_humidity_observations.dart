...
class ChartHumidityObservations extends ConsumerStatefulWidget {
  const ChartHumidityObservations({Key? key}) : super(key: key);

  @override
  ChartHumidityObservationsState createState() => ChartHumidityObservationsState();
}

class ChartHumidityObservationsState extends ConsumerState<ChartHumidityObservations> {


  // bool showAvg = false;

  late List<Color> gradientColors;
  double minHumidity = 100000;
  double maxHumidity = -10000;
  ...
  Widget build(BuildContext context,) {
    ...
    // }
    //
    // setMinAndMaxHumidity(weatherProv);
    ...
    //       child: Row(
    //         children: [
    //           Image.asset('$imagePath/humidity.png', width: 24, height: 24, color: context.colorScheme.tertiary,),
    //           const SizedBox(width: 20,),
    //           Column(
    //             mainAxisAlignment: MainAxisAlignment.start,
    //             crossAxisAlignment: CrossAxisAlignment.start,
    //             children: [
    //               Text('Humidity ${weatherProv.singleWeatherArea!.todayObservations?.observations.last.humidityAvg}%'),
    //               Text('Humidity: ↓ $minHumidity% @ ${Conversion.time(minTime, ref.read(settingsProvider).units)}   ↑ $maxHumidity° @ ${Conversion.time(maxTime, ref.read(settingsProvider).units)}', style: context.style.labelSmall,),
    //             ],
    //           ),
    //         ],
    //       ),
  ...
  LineChartData mainData(WundergroundTodayObservations? todayObservations) {
    ...
    for (int i=0; i<todayObservations!.observations.length; i++){
      ...
      flSpotList.add(FlSpot(i.toDouble(), observation.humidityHigh??0));
    }
  ...
  List<Color> getGradientColors(WeatherAreasProvider weatherProv) {
    List<Color> gradientColors = [Colors.deepOrange, Colors.deepOrange];
    return gradientColors;
  }



  void setMinAndMaxHumidity(WeatherAreasProvider weatherProv) {
    for (int i=0; i<weatherProv.singleWeatherArea!.todayObservations!.observations.length; i++){
      ...
      double highHumidity = observation.humidityHigh ?? 0;
      if (highHumidity<minHumidity){
        minHumidity = highHumidity;
        minTime = observation.obsTimeLocal;
      }
      if (highHumidity>maxHumidity){
        maxHumidity = highHumidity;
